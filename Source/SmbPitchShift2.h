...
void sortedIdx(float *arr, int size, int *result) {
	...
	for (int i = 0; i < size; i++) {
		...
		bool changed = false;
		for (int r = 0; r < lastIdx+1; r++) {
			int rIdx = lastIdx == 0 ? 0 : result[r];
			auto lv = arr[rIdx];
			if (v > lv) {
				for (int j = lastIdx; j >= r; j--) {
					result[j + 1] = result[j];
				}
				result[r] = i;
				lastIdx++;
...
String smbPitchShift(float pitchShift, long numSampsToProcess, long fftFrameSize, long osamp, float sampleRate, float *indata, float *outdata)
/*
...
*/
{
	...
	static float sortedAnaMagn[MAX_FRAME_LENGTH];
	static int sortedAnaMagnIdx[MAX_FRAME_LENGTH];
	float topFreq = 0.0f;
	...
	for (i = 0; i < numSampsToProcess; i++) {
		...
		if (gRover >= fftFrameSize) {
			...
			float frameTopFreq = 0;
			float frameTopMagn = 0;
			...
			if (topFreq == 0) {
				topFreq = frameTopFreq;
			}
			else
			{
				topFreq = (topFreq + frameTopFreq) / 2;
			}

			/*=================== Pitch Detection ========================*/
			
			/*memcpy(sortedAnaMagn, gAnaMagn, sizeof(gAnaMagn));
			std::sort(sortedAnaMagn, sortedAnaMagn + MAX_FRAME_LENGTH, std::greater<float>());
			
			//muito lento 17ms
			for (int idxSorted = 0; idxSorted < MAX_FRAME_LENGTH; idxSorted++)
			{
				float magnSorted = sortedAnaMagn[idxSorted];

				for (int idxMagn = 0; idxMagn < MAX_FRAME_LENGTH; idxMagn++)
				{
					float magn = gAnaMagn[idxMagn];
					if (magn == magnSorted) {
						sortedAnaMagnIdx[idxSorted] = idxMagn;
						break;
					}
				}
				
			}*/
			//sortedIdx(gAnaMagn, fftFrameSize, sortedAnaMagn);
			//topFreq = gAnaFreq[0];
			//DBG("TopFreq: " << topFreq);
